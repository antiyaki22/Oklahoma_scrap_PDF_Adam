...
def extract_address(text):
    try:
        ...
        def clean_text(text):
            # Remove non-ASCII characters and trim extra spaces
            return re.sub(r'[^\x00-\x7F]+', ' ', text).strip()

        text = clean_text(text)
        text = re.sub(r'\s+', ' ', text)

        print("Cleaned Text:", text)  # Debugging output

        try:
            # Parse the address using usaddress
            parsed_address = usaddress.parse(text)
            print("usaddress.parse output:", parsed_address)  # Debugging output

            # Initialize variables to store the best address components
            address_number = None
            street_name = []
            ...
            # Loop through the parsed components and store the most relevant ones
            ...
            # If there's address number and street name, combine them with street type
            ...
            # Ensure only one city, state, and zip code is selected
